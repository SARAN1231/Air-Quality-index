...
function App() {
  ...
  useEffect(() => {
    ...
    onValue(sensorDataRef, (snapshot) => {
      ...
      const timestamps = [];
      const no2Concentrations = [];
      for (const key in sensorData) {
        const data = sensorData[key];
        timestamps.push(data.Timestamp);
        no2Concentrations.push(
          data.Measurements?.Gases?.NO2?.Concentration || 0
        );
      }
      setData({
        ...data,
...
export default function AppWithErrorBoundary_2() {
  return (
    <ErrorBoundary>
      <App />
    </ErrorBoundary>
